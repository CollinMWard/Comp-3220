...
(define interpreter
  (lambda (prog)
    (let ((theEnv (empty-env)))
      ...
      (newline)
       (cond
         ((eqv? (car prog) 'program) (process (cdr prog) theEnv)(display "Done"))
...
(define process
  (lambda (statements myEnv)
    (cond
      ...
      (else
       (let ((result-env (interp (car statements) myEnv)))
         (process (cdr statements) result-env))))))
...
(define exp
  (lambda (myEnv e)
    (cond
     ...
     ((eqv? (car e) '-) (- (exp myEnv (cadr e)) (exp myEnv (caddr e))))
     ((eqv? (car e) '*)
      (* (exp myEnv (cadr e)) (exp myEnv (caddr e))))
     ((eqv? (car e) '/)
      (let ((numerator (exp myEnv (cadr e)))
            (denominator (exp myEnv (caddr e))))
